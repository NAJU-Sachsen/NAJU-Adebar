...
public class StreamsUnitTest {
    ...
    public void testIntersect() {
        Assert.assertArrayEquals("Should only contain odd numbers", oddNumbers.toArray(),
                Streams.intersect(allNumbers.stream(), oddNumbers.stream()).collect(Collectors.toList()).toArray());
    ...
    public void testSubtract() {
        Assert.assertArrayEquals("Should only contain even numbers!", evenNumbers.toArray(),
                Streams.subtract(allNumbers.stream(), oddNumbers.stream()).collect(Collectors.toList()).toArray());

        Stream<Integer> firstSubtraction = Streams.subtract(allNumbers.stream(), oddNumbers.stream());
        Assert.assertEquals("Should contain no more elements!", EMPTY,
                Streams.subtract(firstSubtraction, evenNumbers.stream()).collect(Collectors.toList()).size());
        Assert.assertArrayEquals("Should not have removed any elements", evenNumbers.toArray(),
                Streams.subtract(evenNumbers.stream(), emptyList.stream()).collect(Collectors.toList()).toArray());
    }
    ...
    public void testContains() {
        ...
        Assert.assertFalse("Stream does not contain element!", Streams.contains(evenNumbers.stream(), 7));
