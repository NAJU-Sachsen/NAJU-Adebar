...
public class Validation {
  ...
  private static final String SINGLE_EMAIL_REGEX = "[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z]{2,6}";

  /**
   * Regular expression used to validate email addresses.
   */
  public static final Pattern EMAIL_REGEX =
      Pattern.compile("^" + SINGLE_EMAIL_REGEX + "$", Pattern.CASE_INSENSITIVE);

  public static final Pattern MULTI_EMAIL_REGEX = Pattern.compile(
      "^" + SINGLE_EMAIL_REGEX + "(;" + SINGLE_EMAIL_REGEX + ")*$", Pattern.CASE_INSENSITIVE);
  ...
  public static boolean isEmail(String text) {
    return isEmail(text, true);
  }

  public static boolean isEmail(String text, boolean allowMultiEmail) {
    Assert.notNull(text, "Text to validate may not be null!");
    if (allowMultiEmail) {
      return MULTI_EMAIL_REGEX.matcher(text).find();
    } else {
