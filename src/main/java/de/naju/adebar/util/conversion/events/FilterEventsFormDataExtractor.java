...
public class FilterEventsFormDataExtractor {
    ...
    public FilterEventsFormDataExtractor() {
        this.dateTimeFormatter = DateTimeFormatter.ofPattern(FilterEventsForm.DATE_TIME_FORMAT, Locale.GERMAN);
    }
    ...
    public AddressFilter extractAddressFilter(FilterEventsForm eventsForm) {
        ...
        Address address = new Address(eventsForm.getStreet(), eventsForm.getZip(), eventsForm.getCity());
     ...
     * @throws IllegalStateException if the form does not contain data for such a filter
     */
    public MinimumParticipantAgeFilter extractMinimumParticipantAgeFilter(FilterEventsForm eventsForm) {
    ...
    public ParticipantsLimitFilter extractParticipantsLimitFilter(FilterEventsForm eventsForm) {
        ...
        return new ParticipantsLimitFilter(eventsForm.getParticipantsLimit(), ComparableFilterType.valueOf(eventsForm.getParticipantsLimitFilterType()));
    }
    ...
    public StartTimeFilter extractStartTimeFilter(FilterEventsForm eventsForm) {
        ...
        return new StartTimeFilter(startTime, DateTimeFilterType.valueOf(eventsForm.getStartFilterType()));
    }
    ...
    public Iterable<EventFilter> extractAllFilters(FilterEventsForm eventsForm) {
        ...
        if (hasAddressFilter(eventsForm)) filters.add(extractAddressFilter(eventsForm));
        if (hasEndTimeFilter(eventsForm)) filters.add(extractEndTimeFilter(eventsForm));
        if (hasMinimumParticipantAgeFilter(eventsForm)) filters.add(extractMinimumParticipantAgeFilter(eventsForm));
        if (hasNameFilter(eventsForm)) filters.add(extractNameFilter(eventsForm));
        if (hasParticipantsLimitFilter(eventsForm)) filters.add(extractParticipantsLimitFilter(eventsForm));
        if (hasParticipationFeeFilter(eventsForm)) filters.add(extractParticipationFeeFilter(eventsForm));
        if (hasStartTimeFilter(eventsForm)) filters.add(extractStartTimeFilter(eventsForm));
        return filters;
    }
}
