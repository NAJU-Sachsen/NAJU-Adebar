...
public class ParticipantsList extends AbstractEventInfo implements Iterable<Person> {
	...
	/**
	 * Calculates the ratio of occupied beds. This only works reliably if flexible participation times
	 * are disabled.
	 * <p>
	 * If the accommodation is specified, the ration will be based on it rather than a participants
	 * limit.
	 * <p>
	 * In case there are neither participants limit nor accommodation information, {@code 0} will be
	 * returned.
	 */
	public double getOccupationRatio() {
		if (!hasAccommodationInfo() && !hasParticipantsLimit()) {
			return 0;
		}
		final int participantsCount = getParticipantsCount();
		final Capacity totalCapacity;

		// determine total capacity to use
		if (hasAccommodationInfo()) {
			totalCapacity = Capacity.of(accommodation.getTotalBeds());
		} else {
			totalCapacity = participantsLimit;
		}

		return (double) participantsCount / totalCapacity.getValue();
	}
	...
	public Position putOnWaitingList(@NonNull Person participant) {
		assertEventNotCanceled();
		Assert.notNull(participant, "participant may not be null");
		Assert2.isFalse(isOnWaitingList(participant),
	...
	/**
	 * Checks whether the given participant is on the waiting list (on any position).
	 */
	public boolean isOnWaitingList(@NonNull Person participant) {
		return waitingList.contains(participant);
	}
	...
	public void removeFromWaitingList(@NonNull Person participant) {
		assertEventNotCanceled();
		Assert.notNull(participant, "participant may not be null");
		Assert.isTrue(isOnWaitingList(participant), "Not on waiting list: " + participant);
