...
public class OrganizationController {
	...
	public String addCounselor(@PathVariable("id") Event event,
		...
		if (event.getParticipationInfo().supportsFlexibleParticipationTimes()) {
			counselorInfoBuilder.withParticipationDuring(new ParticipationTime(
					form.generateFirstNightAsLDT(), form.generateLastNightAsLDT(), event.getStartTime()));
		}

		if (participationManager.addCounselor(event, form.getCounselor(),
				counselorInfoBuilder.build()) != Result.OK) {
