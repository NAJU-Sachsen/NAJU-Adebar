...
class EventFormValidator {
	...
	void doValidate(EditEventForm form, Errors errors) {
		...
		try {
			...
			ValidationUtils.invokeValidator(participationInfoFormConverter, form.getParticipationInfo(),
					errors);
		} finally {
			errors.popNestedPath();
		}
	...
	boolean belongingIsValid(EditEventForm form) {
		return form.getBelonging() != null && (form.getLocalGroup() != -1 || form.getProject() != -1);
	}
