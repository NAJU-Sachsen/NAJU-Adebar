...
@Service
public class AccommodationFormConverter implements
    ValidatingEntityFormConverter<ExtendedRoomSpecification, AccommodationForm> {
  ...
  public AccommodationForm toForm(ExtendedRoomSpecification entity) {
    ...
    entity.getRooms().forEach(room ->
        rooms.add(new SimpleRoomForm(
            RoomType.fromGender(room.getGender()), Capacity.of(room.getBedsCount())))
    );

    entity.getFlexRooms().forEach(room ->
        rooms.add(new SimpleRoomForm(RoomType.FLEX, Capacity.of(room.getBedsCount())))
    );

    entity.getFallbackRooms().forEach(room ->
        rooms.add(new SimpleRoomForm(RoomType.FALLBACK, Capacity.of(room.getBedsCount())))
    );
