 ...
 */
public class FilterConnective<Entity, Filter extends AbstractFilter<Entity>> implements AbstractFilter<Entity> {
     ...
     */
    public static <Entity, Filter extends AbstractFilter<Entity>> FilterConnective<Entity, Filter> forFilter(AbstractFilter<Entity> filter) {
    ...
    /**
     * Creates an `AND` connective between the two filters. More precisely if the result of the first filter is
     * R₁ and the result of the second filter is R₂, then {@code F₁ ∧ F₂} will
     * contain all elements {@code e}, where {@code e∈R₁ ∧ e∈R₂}.
     * @param filter the second filter to use
     * @return the resulting connective
     * @throws IllegalStateException if multiple connectives where specified (i.e. {@link #and(AbstractFilter)}
     * or {@link #or(AbstractFilter)} where called before)
     */
    ...
    /**
     * Creates an `OR` connective between the two filters. More precisely if the result of the first filter F₁ is
     * R₁ and the result of the second filter F₂ is R₂, then {@code F₁ ∨ F₂} will
     * contain all elements {@code e}, where {@code e∈R₁ ∨ e∈R₂}.
     * @param filter the second filter to use
     * @return the resulting connective
     * @throws IllegalStateException if multiple connectives where specified (i.e. {@link #and(AbstractFilter)}
