...
public class AddressFilter implements PersonFilter {
     ...
     * @param matchType the way the address should be treated, i.e. whether the address has to match exactly or
     *                  whether only specified fields should be considered
        ...
        switch (matchType) {
            ...
            case IF_DEFINED:
                if (!address.getCity().isEmpty()) {
                    personStream = personStream.filter(p -> p.getAddress().getCity().equals(address.getCity()));
                }
                if (!address.getStreet().isEmpty()) {
                    personStream = personStream.filter(p -> p.getAddress().getStreet().equals(address.getStreet()));
                }
                ...
                if (!address.getAdditionalInfo().isEmpty()) {
                    personStream = personStream.filter(p -> p.getAddress().getAdditionalInfo()
