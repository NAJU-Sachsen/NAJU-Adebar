...
public class PersistentPersonManager implements PersonManager {
    ...
    @Autowired
    public PersistentPersonManager(PersonRepository personRepo, @Qualifier("ro_personRepo") ReadOnlyPersonRepository roRepo, PersonFactory personFactory, ParticipantProfileRepository participantRepo, ActivistProfileRepository activistRepo, ReferentProfileRepository referentRepo, QualificationRepository qualificationRepo) {
        Object[] params = {personRepo, roRepo, personFactory, participantRepo, activistRepo, referentRepo, qualificationRepo};
    ...
    @Override
    public Person createPerson(String firstName, String lastName, String email, boolean participant, boolean activist, boolean referent) {
        Person p = personFactory.buildNew(firstName, lastName, email).create();
        if (participant) {
            p.makeParticipant();
        }
    ...
    public void deactivatePerson(Person person) {
        if (person.isActivist() || person.isReferent()) {
            throw new IllegalStateException("Person may not be deactivated as it is a referent or activist: " + person);
        }
    ...
    /**
     * Replaces the participation profile of a person if specified. Otherwise the database will be cleaned.
     * @param person the person to update
    ...
    protected void updateParticipantProfile(Person person, Person newPerson) {
        if (newPerson.isParticipant()) {
            person.setParticipantProfile(doUpdateParticipantProfile(person, newPerson.getParticipantProfile()));
        } else {
     ...
     */
    protected ParticipantProfile doUpdateParticipantProfile(Person person, ParticipantProfile newProfile) {
    ...
    protected void setQualifications(ReferentProfile profile, List<Qualification> qualifications) {
        try {
            Method changeQualifications = ReferentProfile.class.getDeclaredMethod("setQualifications", List.class);
