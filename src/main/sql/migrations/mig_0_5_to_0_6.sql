...
alter table event_participants
  ...
  add column form_sent bit(1) default true,
...
update event_participants
  set last_night = datediff(
    (select end_time from event where id=event_id),
    (select start_time from event where id=event_id)
  );
...
update event e
  set participants_limit = (select participants_limit from participants_list pl where e.id=pl.id);
