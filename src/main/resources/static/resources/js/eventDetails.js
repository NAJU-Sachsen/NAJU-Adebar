...
function displayMatchingNonActivists(modal, result, eventId) {
  ...
  for (var i = 0; i < result.length; i++) {
    var person = result[i];
    var entry = createPersonListEntry(eventId, person.id, person.name,
...
function createReservationRow(description, slots, email) {
  var descriptionColumn = '<td class="col-md-6 description">' + description
      + '</td>';
  var slotsColumn = '<td class="col-md-2 slots">' + slots + '</td>';
  var emailColumn = '<td class="col-md-3 email"><a href="mailto:' + email + '">'
      + email + '</a></td>';
  var controlsColumn = '<td class="col-md-1 controls"><div class="btn-group btn-group-xs" role="group"><button type="button" class="btn btn-default hover-primary edit-reservation"><span class="glyphicon glyphicon-pencil"></span></button><button type="button" class="btn btn-default hover-danger remove-reservation" data-description="'
      + description
      + '"><span class="glyphicon glyphicon-trash"></span></button></div></td>';
  var row = '<tr class="row reservation">' + descriptionColumn + slotsColumn
      + emailColumn + controlsColumn + '</tr>';
  return row;
}
...
function fetchAddReservationResponse(response, row) {
  var reservationSlots = $('tr.new-reservation').find(
      'input.reservation-slots');
...
$(document).on('click', 'button.edit-reservation', function () {
  ...
  var editDescription = '<td class="col-md-6"><input type="text" class="form-control input-sm reservation-description" placeholder="Beschreibung" value="'
      + description + '" required="required"></td>';
  var editSlots = '<td class="col-md-2"><input type="number" class="form-control text-right input-sm reservation-slots" value="'
      + slots + '" min="1" max="' + maxSlots + '"></td>';
  var editEmail = '<td class="col-md-3"><input type="email" class="form-control input-sm reservation-email" placeholder="Kontakt E-Mail" value="'
      + email + '"></td>';

  var editControls = '<td class="col-md-1" style="vertical-align:middle;"><div class="btn-group btn-group-xs" role="group"><button type="button" class="btn btn-default hover-success save-reservation"><span class="glyphicon glyphicon-ok"></span></button><button type="button" class="btn btn-default hover-danger cancel-edit" data-description="'
      + description + '" data-slots="' + slots + '" data-email="' + email
      + '"><span class="glyphicon glyphicon-remove"></span></button></div></td>';

  var editRow = '<tr class="row reservation-edit">' + editDescription
      + editSlots + editEmail + editControls + '</tr>';

  reservation.before(editRow);
  reservation.remove();

  currentEdit = $('table.reservations').find('tr.reservation-edit').find(
      'button.cancel-edit');
});
...
$('tr.new-reservation').find('button.add').click(function () {
  ...
  var eventId = $('#event-id').val();
  var description = $('tr.new-reservation').find(
      'input.reservation-description').val();
