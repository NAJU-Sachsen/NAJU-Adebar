...
$('#add-person-isNabuMember').click(function () {
  $('#add-person-nabuNr').prop('disabled', function (i, v) {
    return !v;
  });
});
...
$('#filter-gender-type').on('change', function () {
  if ($('#filter-gender-type').find('option:selected').val().toString()
      === 'none') {
...
$('#filter-dob-type').on('change', function () {
  if ($('#filter-dob-type').find('option:selected').val().toString()
      === 'none') {
...
var filterNabuMembershipType = (filterNabuType.find(
    'option:selected').val().toString() === 'ENFORCE');

// show nabu membership number input only if only for nabu members should be filtered
filterNabuType.on('change', function () {
  if ($(this).find('option:selected').val().toString() === 'ENFORCE'
      && !filterNabuMembershipType) {
...
$('#filter-activist-type').on('change', function () {
  if ($('#filter-activist-type').find('option:selected').val().toString()
      !== 'ENFORCE') {
...
$('#filter-activist-juleica-type').on('change', function () {
  if ($('#filter-activist-juleica-type').find(
          'option:selected').val().toString() !== 'ENFORCE') {
...
$('#filter-activist-juleicaDate-type').on('change', function () {
  if ($('#filter-activist-juleicaDate-type').find(
          'option:selected').val().toString() === 'none') {
